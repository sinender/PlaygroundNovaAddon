...
object ExampleAddon : Addon() {
    @InitFun
    private fun init() {
        // This was made by my brother for me to place in files of this plugin that automatically goes into the base packs folder making things much easier.
        val plugin = this.plugin ?: return;
        val logger = plugin.logger
        try {
            val targetDir = File("plugins/Nova/resource_pack/base_packs/base") // Suggestion: make this configurable by the config file.
            if (!targetDir.exists()) {
                targetDir.mkdirs()
    ...
    private fun copyResourcesFromFileSystem(targetDir: File) {
        val plugin = this.plugin ?: return
        val logger = plugin.logger
        val resourcesDir = File("src/main/resources/base_pack")
